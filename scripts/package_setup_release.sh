...
#
# Writes: dist/nightmarecreaturesii-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix nightmarecreaturesii \
  --exe-name Nightmare_Creatures_II \
  --omit-openbios \
  --display-name "Nightmare Creatures II" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Nightmare Creatures II (USA) disc" \
  --runtime-dir mods \
